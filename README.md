...
机组分析准入子域位于 `plant_science` 包，负责机组巡检传感器的设备构建登记、不可变校准协议、测点分片导入、异常测点复核、统计任务租约、分析准入决定和审计报告。该子域不连接传感器硬件，只处理已经结构化的校准记录。统计任务采用租约制：工作进程领取后按可注入时钟心跳续租，持有者租约有效时不会被抢占；租约超时回收、主动失败和放弃都会保留失败次数与最后错误并写入审计链；任务完成或放弃后进入终态，同一输入不会再次执行。
